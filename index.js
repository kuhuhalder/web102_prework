...
function addGamesToPage(games) {
    ...
    for (let game of games) {
        ...
        <div class="game-stats">
          <span class="stat">Backers: ${game.backers} </span>
        </div>`
...
const numContributions = GAMES_JSON.reduce( (total, game) => {return total + game.backers}, 0);
...
function filterFundedOnly() {
    ...
    const funded = GAMES_JSON.filter( (games) => { return games.pledged >= games.goal; } );
    console.log("number of funded games ", funded.length);
...
const unfundedString = numUnfunded === 1 ? "game" : `games`;
const displayStr  = `A total of $${totalRaised.toLocaleString()} has been raised for ${numGames.toLocaleString()} 
${unfundedString}. Currently, ${numUnfunded} ${unfundedString} remains unfunded. We need your help to fund these amazing games!`
